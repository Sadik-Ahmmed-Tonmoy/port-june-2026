...
import { FaLaptopCode, FaReact, FaUserGraduate } from "react-icons/fa6";
...
const TIMELINE_DATA: TimelineItemType[] = [
  ...
  },
  {
    ...
    description:
      "Developed dynamic and responsive web applications while collaborating with senior developers",
    details: [
  ...
  },
  {
    ...
    description:
      "Currently developing and maintaining high-performance web applications",
    details: [
...
const HexNode = ({ icon, color }: { icon: React.ReactNode; color: string }) => (
  <div className="relative w-12 h-12 flex items-center justify-center group">
    ...
    <div
      className={cn(
        "absolute inset-0 bg-gradient-to-tr rounded-xl rotate-45 scale-75 opacity-20 blur-sm",
        color,
      )}
    />
    ...
    <svg
      viewBox="0 0 100 100"
      className="absolute inset-0 w-full h-full fill-white dark:fill-slate-950 stroke-neutral-300 dark:stroke-neutral-700 group-hover:stroke-indigo-600 dark:group-hover:stroke-indigo-400 transition-colors duration-300"
    >
...
const TypeBadge = ({ type }: { type: "education" | "training" | "work" }) => {
  const styles = {
    education: {
      ...
      bg: "bg-purple-100/50 dark:bg-purple-950/40 text-purple-700 dark:text-purple-300 border-purple-200 dark:border-purple-800/30",
    },
    training: {
      label: "Training",
      icon: (
      ...
      bg: "bg-emerald-100/50 dark:bg-emerald-950/40 text-emerald-700 dark:text-emerald-300 border-emerald-200 dark:border-emerald-800/30",
    },
    work: {
      label: "Work Experience",
      icon: <Briefcase className="w-3 h-3 text-sky-655 dark:text-sky-400" />,
      bg: "bg-sky-100/50 dark:bg-sky-950/40 text-sky-700 dark:text-sky-300 border-sky-200 dark:border-sky-800/30",
    },
  };
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1 border px-2 py-0.5 rounded text-[10px] font-medium font-mono uppercase tracking-wider",
        current.bg,
      )}
    >
      {current.icon}
...
const TiltCard = ({
  ...
  return (
    <div className="relative group bg-neutral-100/60 dark:bg-neutral-900/35 border border-neutral-200 dark:border-neutral-850 rounded-2xl p-6 sm:p-8 backdrop-blur-md shadow-lg dark:shadow-2xl hover:border-neutral-350 dark:hover:border-neutral-700/60 transition-all duration-300 overflow-hidden hover:scale-[1.01]">
          ...
          <div className="flex flex-col space-y-5 relative z-10">
            ...
            <div className="flex flex-col space-y-3 pb-4 border-b border-neutral-250 dark:border-neutral-800/60">
              ...
              <div>
                ...
                <h4
                  className={cn(
                    "text-xs sm:text-sm sm:text-base font-bold bg-gradient-to-r bg-clip-text text-transparent mt-1 inline-block",
                    entry.color,
                  )}
                >
                  {entry.subtitle}
    ...
    >
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="text-center max-w-2xl mx-auto mb-16 sm:mb-24 px-4">
          ...
          >
            A timeline of my formal academic qualifications, professional
            trainings, and industrial experiences in full-stack engineering.
          </motion.p>
        </div>
        ...
        <div className="relative max-w-6xl mx-auto flex flex-col items-center">
          ...
          {TIMELINE_DATA.map((entry, index) => {
            ...
            return (
              <TimelineCard key={entry.id} entry={entry} isLeft={isLeft} />
            );
          })}
        </div>
      </div>
